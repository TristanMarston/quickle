...
import { RotateCcw, X } from 'lucide-react'; // X icon in top right of modal
...
const Settings = () => {
    ...
    const { setSettingsModalOpened, darkMode, setDarkMode, hardMode, setHardMode, isRunning, isOver, gamePaused, alertDialog, guessLength, setGuessLength } = context;
                ...
                <div className='flex flex-col gap-3 mb-3'>
                    ...
                    <div className='flex gap-3 items-center'>
                        ...
                        <Slider
                            ...
                            max={7}
                            step={1}
                            onValueChange={(value: number[]) => setGuessLength(value[0])}
                            disabled={isRunning || gamePaused}
                            className={`${isRunning || gamePaused ? 'opacity-50 cursor-not-allowed hover:cursor-not-allowed' : 'opacity-100'}`}
                        />
                    </div>
                </div>
